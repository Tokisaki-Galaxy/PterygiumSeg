...
## 2025-04-27
两个关键变量解释：
MIN_FOREGROUND_RATIO：病灶相关比例，前景patch中只有对应的比例才会被选取
HEALTHY_PTERYGIUM_PATCH_RATIO：健康眼睛比例，健康眼睛的patch数量比病灶眼睛的patch数量（MIN_FOREGROUND_RATIO的比例）

前几天添加的自动阈值探测非常好，附带了在本地对大图的dice检测。之前只有patch级别的dice检测。添加的加权采样也非常有用。
今天发现核心问题在于数据输入，在work2的2、4、13、14等比较中，发现MIN_FOREGROUND_RATIO，HEALTHY_PTERYGIUM_PATCH_RATIO和加权采样非常非常严重影响了模型的训练。

由于观测到实验14效果很好，但是切分patch占用大量时间。kaggle无法缓存这么大的patch（13G），所以考虑降低四分之一图像分辨率。

还有，现在的模型（14）预测和模型（5）预测虽然结果差不多，但是模型（14）预测的很保守，后处理的时候不能用削减了，得用向外膨胀。不过这个倒是不急。

唔，发现了一个非常非常关键的问题。今天引入了debug模式，可以在低分辨率进行训练（1296*864）。发现低分辨率下的dice和hd95显著变高（hd95才300+）。之前全分辨率下最好的是2000+。
这并不是因为debug的时候计算数据时是低分辨率下的原因，代码写的时候已经考虑这个问题，在插值到原始分辨率和蒙版进行比较计算hd95/dice。
经过分析发现原本使用的patch方法是切碎了然后训练。但是对于我的数据集来说，patching的方式是错误的。因为眼睛的病灶是有一定的连贯性的。并且一个病灶很大，一张5184x3456的图像上，一般切成1000+块。导致完全损失了整体上的信息，模型只关注逐个像素的信息。其实我觉得现在在debug模式下提交就挺不错，但是改进patch方法才是治本。


## 2025-04-28
score	ACC	    HD_score	MACROF1	DICE	MACROPRE	HD
7.7067	0.9267	0.4517	    0.9263	0.8931	0.9261	    2072.6373

奇了怪了，今天提交了debug模式的代码，HD95还是2000+。不过整体上相比于之前的nodebug还是好很多的。

今天重新想了一下，似乎跟级联处理有关系。**因为级联处理的模型一错误的归类健康为病灶，或者病灶为健康，会极大的损害HD95.**
为了验证这个想法，我手动更改了今天的提交结果，手动修复了图片的分类。找到了三张错误分类的图片。

在这个过程中，还发现有些蒙版发生了伪影（7张）。

今天累了，而且感觉改动没啥优化，先只做这些把，看看明天提交咋样，如果还是2000+就不弄比赛了。
...
昨天的设想是对的，之前2000+的HD是因为级联分类模型的假阳性和假阴性。
今天提交的结果比昨天好很多。HD95从2000+第一次降到362.4233。dice昨天稍微修补了一下伪影，提升到了0.9191。
明明昨天不太想弄了，但是HD95的提升让我有点动力。
...
## 2025-5-6
今天把模型一的分辨率提升到512x512。确认了之前的事情，ResNet18在确认是否健康上（0和12）很好用，但是在确认病情（1和2）上不好。但是ResNet34在确认病情（12）上很好用。
今天尝试了集成学习的boost，因为现在模型二没啥进展，模型一得好好弄弄。

## 2025-5-7
今天跑了一下，发现boosting的效果并不好。因为最后元模型读入的是基本学习器的特征层，维度大概2000左右，对于lightgbm来说，特征太多了。最好的一次差不多也就0.71。而单个模型输入都有0.9+的准确率。
最后换成了stacking，效果还不错。现在是用最后的softmax之后的概率作为元模型的输入。
分析了一下，这种这可能是因为高维特征对于某些 LightGBM 配置可能不是最优的，而概率值通常是更平滑、维度更低的良好特征。
